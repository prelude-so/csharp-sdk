...
public sealed record class HistoryRetrieveResponse : JsonModel
{
    ...
    /// rather than a person.  * `carrier_not_permitted` - The destination carrier
    /// is one this account does not accept traffic for.  * `client_fingerprint_mismatch`
    /// - The client does not appear to be the platform it identifies itself as.
    /// * `custom_policy` - A rule configured for your account matched this request.
    ///  * `device_emulator` - The request appears to come from an emulator rather
    /// than a physical device.  * `device_not_permitted` - The device platform is
    /// one your account blocks.  * `device_reuse` - One device is driving verifications
    /// for an unusual number of phone numbers.  * `expired_signals` - The SDK signals
    /// were collected too long before the request to still attest to it.  * `fraud_database`
    /// - The phone number is flagged in one or more of the fraud databases Prelude
    /// consults.  * `invalid_signature` - The SDK signature did not verify, so the
    /// request cannot be attributed to the device it claims to come from.  * `ip_concentration`
    ...
    /// associated with abuse.  * `poor_conversion_history` - Traffic resembling this
    /// request rarely completes a verification.  * `proxy_network` - The request
...
sealed class BlockReasonConverter : JsonConverter<BlockReason>
{
    ...
    )
    {
        return JsonSerializer.Deserialize<string>(ref reader, options) switch
        {
            "automation_signature" => BlockReason.AutomationSignature,
            "carrier_not_permitted" => BlockReason.CarrierNotPermitted,
            "client_fingerprint_mismatch" => BlockReason.ClientFingerprintMismatch,
            "custom_policy" => BlockReason.CustomPolicy,
            "device_emulator" => BlockReason.DeviceEmulator,
            "device_not_permitted" => BlockReason.DeviceNotPermitted,
            "device_reuse" => BlockReason.DeviceReuse,
            "expired_signals" => BlockReason.ExpiredSignals,
            "fraud_database" => BlockReason.FraudDatabase,
            "invalid_signature" => BlockReason.InvalidSignature,
            "ip_concentration" => BlockReason.IPConcentration,
            "ip_reputation" => BlockReason.IPReputation,
            "location_mismatch" => BlockReason.LocationMismatch,
            "missing_signals" => BlockReason.MissingSignals,
            "number_range_abuse" => BlockReason.NumberRangeAbuse,
            "poor_conversion_history" => BlockReason.PoorConversionHistory,
            "proxy_network" => BlockReason.ProxyNetwork,
            "repeated_attempts" => BlockReason.RepeatedAttempts,
    ...
    )
    {
        JsonSerializer.Serialize(
            ...
            value switch
            {
                BlockReason.AutomationSignature => "automation_signature",
                BlockReason.CarrierNotPermitted => "carrier_not_permitted",
                BlockReason.ClientFingerprintMismatch => "client_fingerprint_mismatch",
                BlockReason.CustomPolicy => "custom_policy",
                BlockReason.DeviceEmulator => "device_emulator",
                BlockReason.DeviceNotPermitted => "device_not_permitted",
                BlockReason.DeviceReuse => "device_reuse",
                BlockReason.ExpiredSignals => "expired_signals",
                BlockReason.FraudDatabase => "fraud_database",
                BlockReason.InvalidSignature => "invalid_signature",
                BlockReason.IPConcentration => "ip_concentration",
                BlockReason.IPReputation => "ip_reputation",
                BlockReason.LocationMismatch => "location_mismatch",
                BlockReason.MissingSignals => "missing_signals",
                BlockReason.NumberRangeAbuse => "number_range_abuse",
                BlockReason.PoorConversionHistory => "poor_conversion_history",
                BlockReason.ProxyNetwork => "proxy_network",
                BlockReason.RepeatedAttempts => "repeated_attempts",
