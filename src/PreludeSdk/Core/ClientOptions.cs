...
public record struct ClientOptions()
{
    ...
    Lazy<string> _baseUrl = new(() =>
        Environment.GetEnvironmentVariable("PRELUDE_BASE_URL") ?? EnvironmentUrl.Production
    );
    ...
    /// <see cref="IPreludeClient.WithRawResponse"/>, don't perform validation until deserialization
    /// is triggered.</para>
    /// </summary>
    ...
    /// </summary>
    Lazy<string> _apiToken = new(() =>
        Environment.GetEnvironmentVariable("API_TOKEN")
        ?? throw new PreludeInvalidDataException(
            string.Format("{0} cannot be null", nameof(ApiToken)),
            new ArgumentNullException(nameof(ApiToken))
        )
    );
    ...
    {
        readonly get { return _apiToken.Value; }
        set { _apiToken = new(() => value); }
    }
}
