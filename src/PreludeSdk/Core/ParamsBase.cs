...
public abstract record class ParamsBase
{
    ...
    static ParamsBase()
    {
        ...
        var customHeadersEnv = Environment.GetEnvironmentVariable("PRELUDE_CUSTOM_HEADERS");
    ...
    internal static void AddDefaultHeaders(HttpRequestMessage request, ClientOptions options)
    {
        ...
        {
            request.Headers.Add("Authorization", string.Format("Bearer {0}", options.ApiToken));
        }
        request.Headers.Add(
            "X-Stainless-Timeout",
    ...
    static string GetUserAgent() => $"{typeof(PreludeClient).Name}/C# {GetPackageVersion()}";
